...
def get():
    ...
    test = pd.Series(data['value'].values, index=data['time'])[0:500]
    ...
    if len(data) != 0:
        ...
        if option == 'all_flow':
            ...
            predict_data = pd.DataFrame(load_model(model, path, int(pre_time)),columns=["prediction"])
            print(type(predict_data))
            # to mysql
            yconnect = create_engine('mysql+pymysql://root:root@localhost:3306/data_mining_DB?charset=utf8')
            pd.io.sql.to_sql(predict_data, new_table, yconnect, if_exists='replace', index=False, index_label=None)
